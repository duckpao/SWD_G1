...
const createQrPayment = async (userId, orderId) => {
  ...
  const qrUrl = generateVietQRUrl(Math.round(parseFloat(order.final_amount)), content);
...
const confirmPayment = async (userId, orderId) => {
  ...
  // Nếu đơn đang ở trạng thái confirmed, chuyển sang shipping (auto-assign shipper)
  if (order.status === "confirmed" || order.status === "pending") {
    try {
      const shipper = await deliveryRepository.updateOrderAndAssignShipper(orderId);
      await orderRepository.createStatusHistory(orderId, order.status, "shipping", 0, "Auto-assign shipper " + shipper.fullname);
    } catch (e) {
