  ...
  return (
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, background: "#fff", borderBottom: "1px solid #e0e0e0", zIndex: 1000, padding: "0 16px" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", alignItems: "center", height: 60, justifyContent: "space-between" }}>
        <Link to="/" style={{ display: "flex", alignItems: "center", gap: 8, textDecoration: "none", fontWeight: "bold", fontSize: 20, color: "#2e7d32" }}>
          <Store size={28} /> Fruit Shop
        </Link>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }} className="nav-links">
          <Link to="/" style={activeLink("/")}><Package size={18} /> Products</Link>
          {user ? (
            <>
              <Link to="/cart" style={activeLink("/cart")}><ShoppingCart size={18} /> Cart</Link>
              <Link to="/orders" style={activeLink("/orders")}><Package size={18} /> Orders</Link>
            ...
            <>
              ...
              <Link to="/register" style={{ ...linkStyle, background: "#2e7d32", color: "#fff", borderRadius: 8 }}>Register</Link>
            </>
          )}
        </div>
      </div>
