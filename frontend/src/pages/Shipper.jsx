...
export default function Shipper() {
  ...
  useEffect(() => { if (user.role !== "shipper") nav("/"); }, []);
  ...
  const renderAmount = (d) => {
    // COD: hien thi 100%, da chuyen khoan: 0d
    if (d.payment_method !== "COD" && d.payment_status === "paid") {
      return <span style={{ fontWeight: "bold", color: "#999" }}>0đ (Đã thanh toán)</span>;
    }
    return <span style={{ fontWeight: "bold", color: "#2e7d32" }}>{parseInt(d.final_amount).toLocaleString()}đ</span>;
  };
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  ...
                  <p style={{ margin: "2px 0 0", fontSize: 12, color: "#999" }}>
                    {d.payment_method === "COD" ? "COD" : d.payment_status === "paid" ? "Đã chuyển khoản" : d.payment_method || "COD"}
                  </p>
                </div>
